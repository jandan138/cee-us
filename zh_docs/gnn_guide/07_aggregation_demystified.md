...
## 与论文主线的对应：聚合让 world model 具备“对象集合”的泛化能力

论文之所以强调 structured world model，一个关键收益是：

- 只要对象集合（objects）改变了排列，或者对象数量变化，world model 仍然能工作，从而支持 zero-shot 泛化

要做到这一点，模型在“汇总邻居信息”时必须像处理集合一样处理：

- 不依赖对象的顺序（置换不变）
- 在对象数变化时尽量不引入尺度灾难（这就是 mean/sum 选择会影响 zero-shot 的原因之一）

所以看起来很“底层”的 aggregation，其实是让论文的 world model 具备组合泛化能力的关键工程点。
